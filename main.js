 ...
 * @private
 */
function g0(x) { return ROTR(2,  x) ^ ROTR(13, x) ^ ROTR(22, x); }
function g1(x) { return ROTR(6,  x) ^ ROTR(11, x) ^ ROTR(25, x); }
...
function hash(msg, options) {
    ...
    switch (opt.msgFormat) {
        ...
        case 'string':   msg = hexBytesToString(msg);       break;
    ...
    for (let i=0; i<N; i++) {
        ...
        for (let t=0; t<64; t++) {
            const T1 = h + g1(e) + Ch(e, f, g) + K[t] + W[t];
            const T2 =     g0(a) + Maj(a, b, c);
            h = g;
            g = f;
            f = e;
            e = (d + T1) >>> 0;
